  ...
  def oidcCallback(): Action[AnyContent] = Action { implicit request =>
    ...
    wsClient
      ...
      .foreach { resp =>
        ...
        val userName =
          (Json.parse(id_token_decoded) \ configuration.get[String]("oidc.jwt-username")).validate[String].get
        val email = (Json.parse(id_token_decoded) \ configuration.get[String]("oidc.jwt-email")).validate[String].get
